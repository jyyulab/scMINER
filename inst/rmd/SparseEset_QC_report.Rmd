...
p1 <- ggplot(pd,aes_string(x=group,y="nUMI.total",fill=group)) +
      ...
      geom_jitter(size=0.4, alpha=0.6,width=0.5)+
  ...
  if(params$output.cutoff){
    p1<- p1 + geom_hline(yintercept = cfs$umi_cf_lo, data = as.data.frame(cfs$umi_cf_lo),
              ...
              geom_label(aes(1,cfs$umi_cf_lo,label = cfs$umi_cf_lo, vjust = -1),size=2,colour="white")+
              geom_hline(yintercept = cfs$umi_cf_hi, data = as.data.frame(cfs$umi_cf_hi),
                 color="blue",size=2,alpha = 0.3) +
              geom_label(aes(1,cfs$umi_cf_hi,label = cfs$umi_cf_hi, vjust = 1),size=2,colour="white")
    }
...
df<-apply(pData(eset)[,c("nCell","nGene","nUMI.total","percent.mito","percent.spikeIn")],2,summary)

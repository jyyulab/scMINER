...
#' @param color_by Character, name of the column of MICA cluster labels. Default: "`clusterID`".
...
#' @param show.cluster_label Logical, whether to show labels on the plot. Default: `TRUE`.
#' @param fontsize.cluster_label Numeric, font size of the labels. Default: 12.
...
#'
#' @examples
#' p_umap <- MICAplot(input_eset = clustered.eset, color_by = "clusterID", X = "UMAP_1", Y = "UMAP_2", point.size = 0.1)
MICAplot <- function(input_eset,
                     color_by = "clusterID", colors = NULL,
                     X = "UMAP_1", Y = "UMAP_2",
                     ...
                     fontsize.axis_title = 10, fontsize.axis_text = 8)
{
  ...
  p <- ggplot(input, aes(x = input[, X], y = input[, Y], color = input[, color_by])) +
    ...
    labs(title = name.plot_title, x = X, y = Y) +
    theme_classic() +
    theme(
  ...
  if (show.cluster_label) {
    loc <- stats::aggregate(input[, c(X,Y)], by = list(ClusterID = input[, color_by]), mean)
    p <- p + geom_text(data = loc, aes(x = loc[, X], y = loc[, Y], label = loc[, "ClusterID"]),
                       check_overlap = TRUE, na.rm = FALSE, size = fontsize.cluster_label,
                       show.legend = F, inherit.aes = FALSE)
  }

  # add number of cells in each category on legend
  if (class(input$clusterID) != "numerical") {
    input$clusterID <- input[,color_by]
    lgnd <- paste0(names(table(input$clusterID))," (",table(input$clusterID),")")
    ...
    p <- p + guides(col = guide_legend(override.aes = list(size = 10), title = paste0(color_by, "\n(",dim(input)[1] ,")")),
                    ncol = ceiling(length(unique(input$clusterID))/10))
  } else {

...
#'
#' @description
#' This function is used to read the gene expression data from a directory containing three files generated by 10x Genomics: **matrix.mtx**, **barcodes.tsv** and
...
#'
#' @param input_dir Path to the directory containing the 3 files generated by 10x Genomics: **matrix.mtx**, **barcodes.tsv** and **features.tsv** (or **genes.tsv**)
#' @param featureType Character, feature type to use as the gene name of expression matrix: `"gene_symbol"` (the default) or `"gene_id"`.
#' @param removeSuffix Logical, whether to remove the suffix "**-1**" when present in all cell barcodes. Default: `TRUE`.
#' @param addPrefix Character or `NULL`, add a prefix to the cell barcodes, like Sample ID. It is highly recommended to use a prefix containing letters and/or numbers only,
#'    and not starting with numbers. Default: `NULL`.
#'
...
#'
#' @description
#' This function is used to read the gene expression data from the HDF5 file generated by CellRanger pipeline of 10x Genomics. This function can automatically distinguish
#' the data of different modalities (e.g. expression data, ATAC data) and retains the gene expression data only. The `**hdf5r**` package is needed to use this function.
#'
...
#' sparseMatrix <- readInput_10x.h5(h5_file, featureType = "gene_symbol", removeSuffix = TRUE, addPrefix = "demoSample")
...
#'    and not starting with numbers. Default: `NULL`.
#'
#' @return A AnnData object containing `"X"` (a observations x variables data matrix), `"obs"` (data frame of observations), `"var"` (data frame of variables) and more. For more details, please check out <https://anndata.readthedocs.io/en/latest/generated/anndata.AnnData.html>.
#' @export
#'
#' @examples
...
#'
#' @param table_file The table format file (e.g. **txt**, **tsv**, **csv**, and others) which the data are to be read from.
#' @param sep String, The field separator character. Default: `"\t"`.
#' @param is.geneBYcell Logical, whether the table is organized in **gene (row) by cell (column)** format. If `FALSE`, the rows will be treated as cells. Default: `TRUE`.
#' @param removeSuffix Logical, whether to remove the suffix "**-1**" when present in all cell barcodes. Default: `FALSE`.
#' @param addPrefix Character or `NULL`, add a prefix to the cell barcodes, like Sample ID. It is highly recommended to use a prefix containing letters and/or numbers only,
#'    and not starting with numbers. Default: `NULL`.
#'
...
#' sparseMatrix <- readInput_table(table_file, sep = "\t", is.geneBYcell = TRUE, removeSuffix = FALSE, addPrefix = "demoSample")
...
#'
#' @description
#' This function is used to create a folder of the specified project names in the specified project directory as the project space to run scMINER analysis.
#' It also creastes 4 subfolers inside of it: "`DATA`", "`MICA`", "`SJARACNe`" and "`PLOT`".
#'

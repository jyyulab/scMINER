...
if (ERCC_cf == 0) (ERCC_filter=FALSE)
...
png("2_2_Distribution_of_percentage_of_MT_and_SpikeIn_preQC.png",width = 10, height = 6, units = "in", res = 300)
print(p.cell.2)
dev.off()
...
if(params$nUMI_filter!=FALSE){
  if(params$nUMI_filter=="low") {umi_cf_hi = Inf} 
  if(params$nUMI_filter=="high") {umi_cf_lo = 0}
  cell <- which((pd$nUMI.total > umi_cf_lo) & (pd$nUMI.total < umi_cf_hi))
}else { cell<- match(rownames(pd),rownames(pd))}
...
p5 <- ggplot(pd,aes(x=sampleID,y=nUMI.total,fill=sampleID)) +
      geom_violin(scale="width", na.rm=TRUE)+
      geom_boxplot(width=0.08,outlier.size = 0.25, outlier.stroke = 0.5,fill="white")+
      labs(x=" ",y="nUMI",title="") +
      scale_y_continuous(breaks=seq(from=min(pd$nUMI.total),to = max(pd$nUMI.total),by=1500))+
      theme(legend.position="none",plot.title=element_text(size=8))

p6 <- ggplot(pd,aes(x=sampleID,y=nGene,fill=sampleID)) +
      geom_violin(scale="width", na.rm=TRUE)+
      geom_boxplot(width=0.08,outlier.size = 0.25, outlier.stroke = 0.5,fill="white")+
      labs(x=" ",y="nGene",title="") +
      scale_y_continuous(breaks=seq(from=min(pd$nGene),to = max(pd$nGene),by=1000))+
      theme(legend.position="none",plot.title=element_text(size=8))
    
p.cell.3 <- cowplot::plot_grid(p5,p6,nrow=1,ncol=2)
png("3_AfterQC_Distribution_of_nGene_and_nUMI.png",width = 14, height = 6, units = "in", res = 300)
print(p.cell.3)
dev.off()
